...
export function registerAdvancedHandlers(bot) {
  ...
  bot.callbackQuery(/^advanced:warnings:(\d+)$/, async (ctx) => {
    ...
    // update() emas, upsert() — agar filterSettings/warningSettings hali
    // yaratilmagan bo'lsa ham (masalan eski ma'lumot yoki race condition),
    // Prisma P2025 (Record not found) xatosi bilan yiqilmasligi uchun.
    await prisma.warningSettings.upsert({
      where: { groupId: group.id },
      update: { maxWarnings },
      create: { groupId: group.id, maxWarnings },
    });
  ...
  bot.callbackQuery(/^advanced:spamcount:(\d+)$/, async (ctx) => {
    ...
    await prisma.spamSettings.upsert({
      where: { groupId: group.id },
      update: { messageLimit },
      create: { groupId: group.id, messageLimit },
    });
  ...
  bot.callbackQuery(/^advanced:spamwindow:(\d+)$/, async (ctx) => {
    ...
    await prisma.spamSettings.upsert({
      where: { groupId: group.id },
      update: { timeWindowSeconds },
      create: { groupId: group.id, timeWindowSeconds },
    });
  ...
  bot.callbackQuery(/^advanced:emoji:(\d+)$/, async (ctx) => {
    ...
    await prisma.filterSettings.upsert({
      where: { groupId: group.id },
      update: { emojiLimit },
      create: { groupId: group.id, emojiLimit },
    });
  ...
  bot.callbackQuery(/^advanced:caps:(\d+)$/, async (ctx) => {
    ...
    await prisma.filterSettings.upsert({
      where: { groupId: group.id },
      update: { capsPercentLimit },
      create: { groupId: group.id, capsPercentLimit },
    });
  ...
  bot.callbackQuery(/^advanced:slowmode:(\d+)$/, async (ctx) => {
    ...
    await prisma.filterSettings.upsert({
      where: { groupId: group.id },
      update: { slowModeSeconds },
      create: { groupId: group.id, slowModeSeconds },
    });
